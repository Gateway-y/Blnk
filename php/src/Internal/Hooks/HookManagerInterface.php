...
/**
 * HookManagerInterface defines the interface for managing hooks.
 *
 * Port of the Go `HookManager` interface (internal/hooks/types.go); the Go
 * ProcessHookTask(ctx, *asynq.Task) becomes processHookTask() taking the
 * queued task's JSON payload (see RedisHookManager for the queue divergence).
 */
interface HookManagerInterface
{
    ...
    /**
     * @param string|array<string, mixed> $task the queued task payload (JSON string or decoded array)
     */
